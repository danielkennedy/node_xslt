...
FUNCTION(xmlDocToString)
    ...
    EXTERNAL(xsltStylesheetPtr, style, objStylesheet, 0);
    EXTERNAL(xmlDocPtr, doc, objDocument, 0);

    xmlChar *str;
    int len = 0;
    xsltSaveResultToString(&str, &len, doc, style);
    ON_BLOCK_EXIT(xmlFree, str);

    RETURN_SCOPED(JS_str2((const char *)str, len));
END
...
extern "C" void init(Handle<Object> target)
{
    ...
    BIND("xmlDocToString", xmlDocToString);
}
